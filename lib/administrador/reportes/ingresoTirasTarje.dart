...
class _IngresoReportesState extends State<IngresoReportes> {
  ...
  late final RegistroRepoApi reportesApi;
  ...
  void initState() {
    super.initState();
    reportesApi = RegistroRepoApi(apiService);
  ...
  int _obtenerIdBanco(Map<String, dynamic> banco) {
    final value = banco['idBanco'] ?? banco['IdBanco'];
  ...
  Future<void> _cargarDatosIniciales() async {
    try {
      ...
      final totales = await reportesApi.obtenerTotales(
        fecha: widget.fecha,
        idBanco: '0',
      );

      final listaBancos = List<Map<String, dynamic>>.from(bancos);

      final Map<String, double> nuevosTotales = {};

      for (final banco in listaBancos) {
        final nombreBanco = banco['nombreBanco'].toString();
        nuevosTotales[nombreBanco] = 0;
      }
  ...
  Future<void> _abrirIngresoReportes({
